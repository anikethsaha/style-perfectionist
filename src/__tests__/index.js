        ...
        Object.keys(spec)
            ...
            .forEach((s) => {
                ...
                t.deepEqual(
                    result,
                    spec[s],
                    `should output the expected result (${s})`
                );
            });
    });
});
...
function handleRaws(t, opts = {}) {
    return postcss([ensureRed, plugin(opts)])
        .process("h1 { color: blue }")
        .then(({ css }) => {
...
ava(
    "should handle declarations added without raw properties (default)",
    handleRaws
);
ava(
    "should handle declarations added without raw properties (compact)",
    handleRaws,
    { format: "compact" }
);
ava(
    "should handle declarations added without raw properties (compressed)",
    handleRaws,
    { format: "compressed" }
);
